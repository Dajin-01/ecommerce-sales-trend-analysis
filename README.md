...
본 프로젝트는 단순 매출 시각화가 아니라, **연말 수요 대응**, **고액 비회원 추정 고객의 회원 전환**, **하절기 재구매 활성화**라는 의사결정에 필요한 근거를 도출하는 것을 목표로 했습니다.
...
따라서 단순히 전체 매출을 보는 것보다, **월별 매출 흐름**, **고객수와 주문건수 변화**, **비회원 추정 거래의 매출 기여도**, **고객 재구매 패턴**을 함께 분석할 필요가 있었습니다.

### Business Questions

1. 매출은 실제로 성장하고 있는가?
2. 어떤 시기에 고객수와 거래건수가 증가하는가?
3. 비회원 추정 거래는 매출에 어떤 영향을 주는가?
...
비회원 추정 거래 비중은 낮더라도, 특정 기간에는 고액 비회원 추정 거래가 전체 매출에 큰 영향을 줄 수 있다.
...
- `CustomerID`가 없는 거래는 비회원 추정 거래로 분류
...
### Finding 2. 비회원 추정 거래 비중은 낮지만 연말 매출 기여도가 큼
